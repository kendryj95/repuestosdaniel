...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            "aoColumns": [
                {"bSortable": false, "mRender": checkbox}, {"bSortable": false,"mRender": img_hl}, {"bSortable": false,"bVisible": false}, null, null, { "sWidth": "20%" }, null, { "sWidth": "50%" }, <?php if($Owner || $Admin) { echo '{"bSortable": false,"bVisible": false}, {"mRender": currencyFormat},{"mRender": currencyFormat},'; } else { if($this->session->userdata('show_cost')) { echo '{"bSortable": false,"bVisible": false},';  } if($this->session->userdata('show_price')) { echo '{"mRender": currencyFormat},{"mRender": currencyFormat},';  } } ?> {"mRender": formatQuantity}, {"bSortable": false}
            ]
            ...
            {column_number: 4, filter_default_label: "[<?=lang('piece_brand');?>]", filter_type: "text", data: []},
            {column_number: 5, filter_default_label: "[<?=lang('category');?>]", filter_type: "text", data: []},
            {column_number: 6, filter_default_label: "[<?=lang('car_brand');?>]", filter_type: "text", data: []},
            ...
            $.ajax({
                ...
                success: function(scdata){
                    ...
                    if (scdata != null) {
                        ...
                        $("#subcategory_filter_select").select2("destroy").empty().attr("placeholder", "<?= lang('no_subcategory') ?>").select2({
                    ...
                    setTimeout(function(){
                        $("#subcategory_filter_select").val("").trigger('change');
                    }, 300);
                },
                error: function (data) {
                    console.log("error");
            ...
            $.ajax({
                ...
                success: function(scdata){
                    ...
                    if (scdata != null) {                        
                        ...
                        alert("El servidor no retornó motores para esta marca");
            ...
            <div class="col-lg-12">
                ...
                <div class="row">
                   <!-- <form class="bv-form"> -->
                        ...
                        <div class="col-md-3" id="subcategory_filter_div" style="display: none;">
                            <div class="form-group">
                    ...
                    <!-- <form class="bv-form"> -->
                        <div class="col-md-4">
                            <select id="selected_items_action" class="form-control">
                                ...
                                <option value="showUpdateCostPorcentajeModal" selected><?= lang('update_costs'); ?> con Porcentaje</option>
                            </select>
                        </div>
                        <div class="col-md-4">
...
<div class="modal" tabindex="-1" role="dialog" id="modalUpdateCostPorcentajeBulk">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><?= lang('update_costs') ?> con Porcentaje</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <form id="myForm3">
            <div class="form-group">
                ...
                <select id="tipoPorcentaje" name="tipoPorcentaje" class="form-control">
                     <option value="mas" selected>Aumento</option>
                     <option value="menos">Descuento</option>
                </select>
                <br><br>
                <label>
                    Porcentaje (<?= lang('major_than') ?> 0)
                </label>
                <input type="number" id="cost-p" name="porcentaje" class="form-control" max="10000000000" min="0" maxlength="3">
            </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
        <button type="button" class="btn btn-primary" onclick="saveChanges3()">Guardar</button>
      </div>
    </div>
  </div>
</div>
<script type="text/javascript">
    ...
    function execMultiSelectAction(){
        ...
        switch(option){
            ...
            case "showUpdateCostPorcentajeModal": showUpdateCostPorcentajeModal(); break;
            default: break;
        }
    }
    ...
    function showUpdateCostPorcentajeModal(){
        $('#modalUpdateCostPorcentajeBulk').modal();
    }
    ...
    function saveChanges3(){
        const formValues = $('#myForm3').serializeArray();
        const formData = new FormData(document.getElementById('myForm3'));
        const selectedItems = $('input[name="val[]"]').serializeArray();
        console.log(formValues);
        if($('#cost-p').val() == "" || Number($('#cost-p').val()) < 0 || Number($('#cost-p').val()) > 100000000){
            bootbox.alert('<?= lang('invalid_field') ?>:' + ' <?= lang('cost') ?>');
            $('#cost-p').focus();
            return;
        }
        ...
        $.ajax({
            type: "post",
            data: formData,
            url: "<?= admin_url('products/update_cost_bulk_porcentaje') ?>",
            dataType: "json",
            processData: false,
            contentType: false,
            cache: false,
            ...
            $('#modal-loading').hide();
            $('#modalUpdateCostPorcentajeBulk').modal('hide');
            if(oTable !== undefined && oTable !== null)
                oTable.fnReloadAjax();
            console.log("always");
        });
