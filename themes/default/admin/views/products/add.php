...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#category_ml_2").select2("destroy").empty().attr("placeholder", "<?= lang('select_categoryml1_to_load') ?>").select2({
            placeholder: "<?= lang('select_categoryml1_to_load') ?>", minimumResultsForSearch: 7, data: [
                {id: '', text: '<?= lang('select_categoryml1_to_load') ?>'}
            ]
        });
        $("#category_ml_3").select2("destroy").empty().attr("placeholder", "<?= lang('select_categoryml2_to_load') ?>").select2({
            placeholder: "<?= lang('select_categoryml2_to_load') ?>", minimumResultsForSearch: 7, data: [
                {id: '', text: '<?= lang('select_categoryml2_to_load') ?>'}
            ]
        });
            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        ...
                        <?= form_input('second_name', set_value('second_name'), 'class="form-control tip" id="second_name" autocomplete="off"'); ?>
                    </div>
                    ...
                    <div class="form-group all">
                        ...
                        echo form_dropdown('brand2', $br, (isset($_POST['brand2']) ? $_POST['brand2'] : ($product ? $product->brand2 : '')), 'class="form-control select" id="brand" placeholder="' . lang("select") . " " . lang("brand") . '" style="width:100%"')
                        ?>
                    </div>
                    <div class="form-group all">
                        <?= lang("brand", "brand") ?> #3
                        ...
                        echo form_dropdown('brand3', $br, (isset($_POST['brand3']) ? $_POST['brand3'] : ($product ? $product->brand3 : '')), 'class="form-control select" id="brand" placeholder="' . lang("select") . " " . lang("brand") . '" style="width:100%"')
                        ?>
                    </div>
                    <div class="form-group all">
                    ...
                    <?php if (isset($access_token)): ?>
                        <div class="form-group all">
                            <?= lang("category_ml", "category_ml") ?>
                            <?php
                            $cat = [];
                            $cat[''] = "";
                            foreach ($categories_ml as $category) {
                                $cat[$category->id] = $category->name;
                            }
                            echo form_dropdown('category_ml', $cat, '', 'class="form-control select" id="category_ml_1" placeholder="' . lang("select") . " " . lang("category") . ' ML" required="required" onchange="categoriesML(this.value,\'category_ml_2\',this.id, 1)" style="width:100%"')
                            ?>
                        </div>
                        
                        <div class="form-group all">
                            <?= lang("category_ml_2", "category_ml_2") ?>
                            <?php
                            echo form_input('category_ml_2', '', 'class="form-control" id="category_ml_2" onchange="categoriesML(this.value,\'category_ml_3\',this.id, 2)" placeholder="' . lang("select_categoryml1_to_load") . '"');
                            ?>
                        </div>
                        
                        <div class="form-group all">
                            <?= lang("category_ml_3", "category_ml_3") ?>
                            <?php
                        ...
                        <div class="form-group all">
                            <?= lang("brand_ml", "brand_ml") ?>
                            <?php
                            echo form_input('brand_ml', '', 'class="form-control" id="brand_ml"  placeholder="' . lang("select_categoryml_to_load") . '"');
                            ?>
                        </div>
                    ...
                    <div class="form-group all" style="opacity: 0;">
                        <?= lang("product_price", "price") ?>
                        <?= form_input('price', '1', 'class="form-control tip" id="price" required="required"') ?>
                    </div>
                    <!--div class="form-group all">
                        <?= lang("product_price", "price") ?>
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                        <input name="mayorista" type="checkbox" class="checkbox" id="mayorista" value="1" <?= isset($_POST['mayorista']) ? 'checked="checked"' : '' ?>/>
                        <label for="mayorista" class="padding05">Web Vitobello Mayorista</label>
                    </div>
                    ...
                    <div class="row" id="extras-con">
                        <div class="col-md-4">
                            <div class="form-group all">
                                <?= lang('Recargo VR 1', 'cf7') ?>
                                <?= form_input('cf7', (isset($_POST['cf7']) ? $_POST['cf7'] : ($product ? $product->cf7 : '')), 'class="form-control tip" id="cf7"') ?>
                            </div>
                        </div>
                          <div class="col-md-4">
                            <div class="form-group all">
...
<?php
$names = array();
foreach ($second_names as $key => $name) {
    $names[] = $name->name;
}
?>
<script type="text/javascript">
    ...
    */
    ...
    function categoriesML(v, ele, id_ele, category) {
        $('#modal-loading').show();
        resetSelectsCategoriesML(category);
        if (v) {
            var options = [];
            $.ajax({
                url: "https://api.mercadolibre.com/categories/" + v,
                type: "GET",
                dataType: "json",
                success: function (scdata) {
                    if (scdata != null) {
                        if (scdata.settings.status == "enabled" && scdata.settings.listing_allowed === true) {
                            $("#"+ele).select2("destroy").empty().attr("placeholder", "<?= lang('no_category_ml') ?>").select2({
                                placeholder: "<?= lang('no_category_ml') ?>",
                                minimumResultsForSearch: 7,
                                data: [{id: '', text: '<?= lang('no_category_ml') ?>'}]
                            });
                        } else if (scdata.children_categories.length > 0) {
                            options.push({id: '', text: '<?= lang('select_category_ml') ?>'});
                            for (const item of scdata.children_categories) {
                                options.push({id: item.id, text: item.name});
                            }
                            
                            setTimeout(() => {
                                $("#"+ele).select2("destroy").empty().attr({
                                    placeholder: "<?= lang('select_category_ml') ?>",
                                    name: "category_ml",
                                    required: "required"
                                }).select2({
                                    placeholder: "<?= lang('select_categoryml_to_load') ?>",
                                    minimumResultsForSearch: 7,
                                    data: options
                                }); 
                                
                            }, 300);
                            $("#"+id_ele).removeAttr('name').removeAttr('required');
                        } else {
                            $("#"+ele).select2("destroy").empty().attr("placeholder", "<?= lang('no_category_ml') ?>").select2({
                                placeholder: "<?= lang('no_category_ml') ?>",
                                minimumResultsForSearch: 7,
            ...
            $("#"+ele).select2("destroy").empty().attr("placeholder", "<?= lang('select_categoryml_to_load') ?>").select2({
                placeholder: "<?= lang('select_categoryml_to_load') ?>",
                minimumResultsForSearch: 7,
                data: [{id: '', text: '<?= lang('select_categoryml_to_load') ?>'}]
            });
        }
        $('#modal-loading').hide();
    }

    function resetSelectsCategoriesML(select) {
        switch (select) {
            case 1:
                $("#category_ml_2").select2("destroy").empty().attr("placeholder", "<?= lang('select_categoryml1_to_load') ?>").select2({
                    placeholder: "<?= lang('select_categoryml1_to_load') ?>", minimumResultsForSearch: 7, data: [
    ...
    function loadBrandsML(category) {
        if (category) {
            $.ajax({
                ...
                success: function (attributes) {
                    if (attributes != null) {
                        var brand = attributes.find(function(obj){
                            return obj.id === "BRAND" && obj.values && obj.values.length > 0;
                        });
                        var brandOptions = [];
                        if (brand != null) {
                            brandOptions.push({id: ``, text: "<?= lang('select_brand_ml') ?>"})
                            for (const item of brand.values) {
                                brandOptions.push({id: `${item.id}_${item.name}`, text: item.name})
                            }

                            setTimeout(() => {
                                $("#brand_ml").select2("destroy").empty().attr("placeholder", "<?= lang('select_brand_ml') ?>").select2({
                                    placeholder: "<?= lang('select_brand_ml') ?>", minimumResultsForSearch: 7, data: brandOptions
                                });
                            }, 300);
                        } else {
                                $("#brand_ml").select2("destroy").empty().attr("placeholder", "<?= lang('no_brand_ml') ?>").select2({
                                    placeholder: "<?= lang('no_brand_ml') ?>", minimumResultsForSearch: 7, data: [
                                        {id: '', text: '<?= lang('no_brand_ml') ?>'}
                                    ]
                                });
                        }
                    } else {
