        ...
        <div class="modal-body">
            ...
            <?php if ($logo) { ?>
                <div class="text-center" style="margin-bottom:20px;">
                    <img src="<?= base_url() . 'assets/uploads/logos/negro/' . $biller->logo; ?>"
                    ...
                    ?>
                        ...
                        <tr>
                            ...
                            <td style="vertical-align:middle;">
                                    <?= ($row->product_name ? $row->product_name : lang("none")) . ' - ' . ($row->category ? $row->category : lang("none")) . ' - ' . ($row->brand ? $row->brand : lang("none")) . ' - ' . ($row->cf1 ? $row->cf1 : lang("none")); ?>
                                <!--?= $row->second_name ? '<br>' . $row->second_name : ''; ?-->
                                <?= $row->details ? '<br>' . lang("details: ") . ': ' . $row->details : ''; ?>
                                <?= $row->serial_no ? '<br>' . lang("serial") . ': ' . $row->serial_no : ''; ?>
                                </td>
                            <!--td style="vertical-align:middle;">
                                <?= lang("name") . ': ' . ($row->product_name ? $row->product_name : lang("none")) . ' - ' . lang("category") . ': ' . ($row->category ? $row->category : lang("none")) . ' - ' . lang("brand") . ': ' . ($row->brand ? $row->brand : lang("none")) . ' - ' . lang("Cf1") . ': ' . ($row->cf1 ? $row->cf1 : lang("none")); ?>
                                <?= $row->second_name ? '<br>' . lang("second_name") . ': ' . $row->second_name : ''; ?>
                                <?= $row->details ? '<br>' . lang("details: ") . ': ' . $row->details : ''; ?>
                                <?= $row->serial_no ? '<br>' . lang("serial") . ': ' . $row->serial_no : ''; ?>
                            </td-->
                            <?php if ($Settings->indian_gst) { ?>
