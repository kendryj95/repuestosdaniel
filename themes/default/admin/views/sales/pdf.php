...
<div id="wrap">
    <div class="row">
        <div class="col-lg-12">
            <div class="text-center" style="">ORIGINAL
                    <div style="font-size: 20px;"><div style="border-style: double; width: 100px; margin-left: 43%; font-size: 40px;"><?php if ($inv->id_tipo_comprobante_afip == '1') {
                            echo 'A'; }
                            else if ($inv->id_tipo_comprobante_afip == '6') { echo 'B'; 
                        }
                            else { echo 'X'; 
                            }
                             ?>
                    ...
                    </div>  <?php if ($inv->id_tipo_comprobante_afip == '1') {
                            echo 'COD. 01'; }
                            else if ($inv->id_tipo_comprobante_afip == '6') { echo 'COD. 06'; 
                        }
                            else { echo ''; 
                            }
                             ?></div>
        ...
        <div class="col-lg-12">
            <?php if ($logo) {
                ...
                ?>
                <div class="text-left" style="margin-bottom:20px; position: absolute; margin-top: -120px; margin-left: 10px;">
                    <img src="<?= $base64; ?>" alt="<?= $biller->company != '-' ? $biller->company : $biller->name; ?>" width="180">
                </div>
                <div class="text-right" style="margin-bottom:20px; position: absolute; margin-top: -120px; margin-right: 10px;">
                    <div class="bold">
                        Factura Nro:  00006-000000<?= $inv->numero_comprobante_afip; ?> <br>
...
<br><br><br>
                <div class="col-xs-7">
                    <h2 class="">Razón Social: <br> -</h2>
                   Av., Buenos Aires <br>
                   Cond. IVA: Responsable Incripto <br>
                   CUIT: - <br>
                   Ingresos Brutos: - <br>
                ...
                <div class="col-xs-5">
                    ...
                    <h2 class="">Datos del Cliente:  <br><?= $customer->company && $customer->company != '-' ? $customer->company : $customer->name; ?></h2>
                    <?= $customer->company && $customer->company != '-' ? '' : '' . $customer->name; ?>
                    <?php
                        echo $customer->address . '<br />' . $customer->city . ' ' . $customer->postal_code . ' ' . $customer->state . '<br />' . $customer->country;
                        echo '<p>';
                        if ($customer->tipo_cliente != '-' && $customer->tipo_cliente != '') {
                            echo '<br>' . 'Cond. IVA' . ': ' . $customer->tipo_cliente;
                        }
                        echo '<br> Cond. Venta: Contado';
                        if ($customer->vat_no != "-" && $customer->vat_no != "") {
                            echo "<br>" . 'CUIT' . ": " . $customer->vat_no;
                        }
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <tr>
                        ...
                        ?>
                        <th><?= lang('subtotal'); ?> C/ Impuestos</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php $r = 1;
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                <td style="text-align:right; width:100px;">
                                    ...
                                    <?= $this->sma->formatMoney($row->unit_price-$row->item_tax/$row->unit_quantity); ?>
                                </td>
                                <?php
                                    if ($Settings->tax1 && $inv->product_tax > 0) {
                                        echo '<td style="width: 90px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 ? '<small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' : '') . $this->sma->formatMoney($row->item_tax/$row->unit_quantity) . '</td>';
                                    }
                                    if ( $Settings->product_discount && $inv->product_discount != 0) {
                    ...
                    <tr>
                        <td colspan="<?= $col; ?>"
                            style="text-align:right; font-weight:bold;">Importe Total
                            (<?= $default_currency->code; ?>)
                        </td>
            ...
            <div class="clearfix"></div>
                ...
                <div class="clearfix"></div>
                        ...
                        CAE: <?= $inv->cae_afip; ?><br>
                        Vencimiento CAE: <?= $inv->fecha_vencimiento_cae; ?><br>
                <div class="clearfix"></div>
