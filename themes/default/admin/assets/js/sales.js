...
localStorage.setItem('slpayment_status','paid');
...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var quantity_stock = item.row.quantity_real;
            ...
            var pr_tax = item.tax_rate != null && item.tax_rate != undefined ? item.tax_rate : 0;
            var pr_tax_val = 0, pr_tax_rate = 0;
            ...
            tr_html += '<td class="text-right">'+ parseInt(quantity_stock) +'</td>';
            ...
            if (site.settings.tax1 == 1) {
                var tax = pr_tax.id != null && pr_tax.id != undefined ? pr_tax.id : 0
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + tax + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (parseFloat(pr_tax_rate) != 0 ? '(' + formatDecimal(pr_tax_rate) + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span></td>';
            }
