...
class Products_model extends CI_Model
{
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $access_token = null)
    {
        $category_ml = $data['category_ml'];
        $brand_ml = $data['brand_ml'];
        unset($data['category_ml']);
        unset($data['brand_ml']);
        if ($this->db->insert('products', $data)) {
            ...
            */
            if ($access_token != null) {
                ...
                $this->db->update('products', array("product_ml" => $ml['id']), array('id' => $product_id));
            }
            // --------------------------------------
            return true;
        }
    ...
    /* ------------------------------ added By Rafa at 17/05/2019 -------------------------- */

    public function updateProductDiscounts($id, $data)
    {
        return $this->db->update('products', $data, array('id' => intVal($id)));
    }

    public function updateProductCost($id, $cost)
    {
        return $this->db->update('products', array("cost" => $cost), array('id' => floatVal($id)));
    }

    public function updateProductPorcentaje($id, $cost, $tipo)
    {
        $query = $this->db->query("select * from sma_products where id='".$id."'");
        ...
        {
            if($tipo=='mas'){
                $data = (($row->cost*$cost)/100)+$row->cost;
            } else if($tipo=='menos'){
                $data = $row->cost-(($row->cost*$cost)/100);
            } 
        }
        return $this->db->update('products', array("cost" => $data), array('id' => intVal($id)));

    }

    public function getAllBrands()
    {	
    	$this->db->select('id as id, name as text')->order_by('name');
...
/* --------------------- added by rafa ---------------------------- */
	public function getProductsIdsForVariant($variant_id)//this is a variant
    {
    	if($variant_id == null)
    		return [];
    	//we use the name of the variant to make a search
    	$variant_name = $this->getVariantName($variant_id);
        ...
        $this->db->select($this->db->dbprefix('products').".id")
            ...
            ->where('product_variants.name', $variant_name);
    ...
    public function getVariantName($variant_id){
    	...
    	$variant_name = "";
    	$this->db->select('name')->from('product_variants')->where('id', $variant_id);
    	$q = $this->db->get();
    	if ($q->num_rows() > 0) {
    		foreach (($q->result()) as $row) {
    ...
    public function getAllMotorsByBrand($brand_id = NULL)//this is a variant
    {
        $this->db->select("{$this->db->dbprefix('product_variants')}.name as text, {$this->db->dbprefix('product_variants')}.id as id")
            ->join('products', 'products.id=product_variants.product_id', 'left')
            ->group_by('product_variants.name');
        if($brand_id)
    ...
    public function getAllMotors()//this is a variant
    {
        $this->db->select("name as text, id")
            ->group_by('name');
        $q = $this->db->get('product_variants');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
...
/* --------------------- end rafa code ---------------------------- */
    ...
    public function getProductsML()
    {
        $sql = "SELECT * FROM sma_products WHERE product_ml IS NOT NULL";
        $q = $this->db->query($sql);
        $data = [];
        foreach (($q->result()) as $row) {
            $data[] = $row;
