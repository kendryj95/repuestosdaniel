...
class Site extends CI_Model
{
    ...
    /* ADDED BY RAFA */
    public function getAllSecondNames() {
        $this->db->select('products.second_name as name')
            ->group_by('products.second_name');
    ...
    /* ADDED BY RAFA */
    public function updateProductCost($product_id, $cost) {        
        
        return ($this->db->update('products', array('cost' => floatVal($cost)), array('id' => intVal($product_id))));
    }
    /* END RAFA CODE */
    ...
    public function calculateAVCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        $tax_rate = $product->tax_rate ? $this->getTaxRateByID($product->tax_rate) : 0;
    ...
    function calculateTax($product_details = NULL, $tax_details, $custom_value = NULL, $c_on = NULL) {
        ...
        $id = $tax_details ? $tax_details->id : 0;
        return array('id' => $id, 'tax' => $tax, 'amount' => $tax_amount);
    }
