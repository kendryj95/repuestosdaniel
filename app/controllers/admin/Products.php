...
class Products extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        ...
        //added by rafa at 18/05/2019
        $this->data['categories'] = $this->products_model->getSubCategories();
        $this->data['brands'] = $this->products_model->getAllBrands();
        $this->data['motors'] = $this->products_model->getAllMotors();
        //end rafa code
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : ($this->input->post('supplier') ? $this->input->post('supplier') : '');
        /* ---- added by rafa -----*/
        $category = $this->input->get('category') ? $this->input->get('category') : ($this->input->post('category') ? $this->input->post('category') : '');
        $subcategory = $this->input->get('subcategory') ? $this->input->get('subcategory') : ($this->input->post('subcategory') ? $this->input->post('subcategory') : '');
        $brand = $this->input->get('brand') ? $this->input->get('brand') : ($this->input->post('brand') ? $this->input->post('brand') : '');
        $motor = $this->input->get('motor') ? $this->input->get('motor') : ($this->input->post('motor') ? $this->input->post('motor') : '');
        $variant_name = "";
        if ($motor != '')
            $variant_name = $this->products_model->getVariantName($motor);
        /* ------ end rafa code -----------*/
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.pricevr as pricevr, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, wp.rack as rack, alert_quantity", FALSE)
            ->from('products');
            ...
            /*$this->datatables
                ...
                /* ---- new columns ---- */
                $this->datatables
                ->select($this->db->dbprefix('products').".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.second_name as sname, {$this->db->dbprefix('categories')}.name as cname, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('products')}.cf1 as description, {$this->db->dbprefix('products')}.cost as cost, {$this->db->dbprefix('products')}.price as price, {$this->db->dbprefix('products')}.pricevr as webprice, COALESCE({$this->db->dbprefix('products')}.quantity, 0) as quantity", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->join('brands', 'products.brand=brands.id', 'left');
                if($motor != '')
                    $this->datatables->join('product_variants', 'products.id=product_variants.product_id', 'left');
                /*if($subcategory != '')
                    $this->datatables->join('categories', 'products.subcategory_id=categories.id', 'left')*/
                /* --- end new columns ---- */        
        ...
        if (!$this->Owner && !$this->Admin) {
            ...
            if (!$this->session->userdata('show_price')) {
                ...
                $this->datatables->unset_column("web_price");
            }
        }
        if ($supplier) {
        ...
        if ($subcategory != ''){
            $this->datatables->where('subcategory_id', $subcategory);
        }

        if($motor != ''){
            $this->datatables->where('product_variants.name', $variant_name);
        }

        if ($brand != '') {
        	/* $this->datatables->where('brand', $brand);
        	$this->datatables->or_where('brand2', $brand);
            $this->datatables->or_where('brand3', $brand); */
            $condicion_brand = " (brand = $brand OR brand2 = $brand OR brand3 = $brand)";
            $this->datatables->where($condicion_brand,NULL,FALSE);
        }
    ...
    function add($id = NULL)
    {
        ...
        * CREADO POR KENDRY
        */
        $access_token = $this->input->post('access_token') ? $this->input->post('access_token') : null;
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $access_token)) {
            ...
            * CREADO POR KENDRY
            */
            if ($this->input->get('meli')) {
                if ($this->session->userdata('ml_accesstoken')) {
                    $accessToken = $this->session->userdata('ml_accesstoken');
                    $now = strtotime(date('Y-m-d H:i:s'));
                    $expires_access_token = $this->session->userdata('expires_accesstoken');
                    if ($now > $expires_access_token) {
                        $accessToken = $this->meli->getAccessToken($accessToken['refresh_token']);
                        if (!isset($accessToken['access_token'])) {
                            $this->session->set_flashdata('error', 'Ha ocurrido un error al intentar obtener la autorización de usar la API de MercadoLibre.');
                            admin_redirect("products/add");
                        }
                        $this->session->set_userdata('ml_accesstoken',$accessToken);
                        $expire_token = strtotime("+$accessToken[expires_in] seconds",strtotime(date('Y-m-d H:i:s')));
                        $this->session->set_userdata('expires_accesstoken',$expire_token);
                        $this->data['access_token'] = $accessToken['access_token'];
                    } else {
                        $this->data['access_token'] = $accessToken['access_token'];
                    }
                }
                $this->data['categories_ml'] = $this->meli->getCategories();
            }
            ...
            $this->data['second_names'] = $this->site->getAllSecondNames();
    ...
    /* -------------------------------- added by Rafa on 17/05/2019 ----------------- */

    function update_price_bulk(){
        $this->sma->checkPermissions('edit', true);
        ...
        if(!$this->input->post('items[]')){
            $error = 1;
            $msg = lang('no_products_selected');
        }else{

            for($i = 2; $i < 7; $i++){
                if($this->input->post('cf' . $i))
                    $data["cf$i"] = $this->input->post('cf' . $i);
            }

            if(empty($data)){
                $error = 1;
                $msg = lang('no_data_received');
            }else{
                $this->db->trans_begin();
                foreach ($items as $key => $productId) {
                    ...
                    if (!$productId || !$product) {
                        ...
                        $error = 1;
                        $msg = sprintf(lang('product_not_found'), $productId);
                        break;
                    }
                    
                    if (!$this->products_model->updateProductDiscounts($productId, $data)) {
                        $this->db->trans_rollback();
                        $error = 1;
                        $msg = sprintf(lang('error_on_update_product'), $productId);
                        break;
                    }
                }
                ...
                if(!$error){
                    ...
                    $msg = sprintf(lang('products_updated'), count($items));
                    $error = 0;
                }
            }
        }    
        if($this->input->is_ajax_request()) {
            $this->sma->send_json(array('error' => $error, 'msg' => $msg, "data" => $data, "items" => $items, "products" => $products, "ids" => $ids));
        }else{
            $aux = $error ? 'error' : 'message';
            $this->session->set_flashdata($aux, $msg);
            redirect($_SERVER["HTTP_REFERER"]);
        }
    }

    function update_cost_bulk(){
        $this->sma->checkPermissions('edit', true);
        $items = $this->input->post('items[]');
        $data = [];
        $ids = array();
        ...
        if(!$this->input->post('items[]')){
            ...
            if($cost == null || empty($cost) || $cost < 0){
                $error = 1;
                $msg = lang('no_data_received');
            }else{
                $this->db->trans_begin();
                foreach ($items as $key => $productId) {
                    ...
                    if (!$this->products_model->updateProductCost($productId, $cost)) {
                        $this->db->trans_rollback();
                        $error = 1;
                        $msg = sprintf(lang('error_on_update_product'), $productId);
                        break;
    ...
    function update_cost_bulk_porcentaje(){
        $this->sma->checkPermissions('edit', true);
        $items = $this->input->post('items[]');
        $data = [];
        $ids = array();
        ...
        if(!$this->input->post('items[]')){
            ...
            $cost = $this->input->post('porcentaje');
            $tipo = $this->input->post('tipoPorcentaje');           
            ...
            if($cost == null || empty($cost) || $cost < 0){
                ...
                foreach ($items as $key => $productId) {
                    ...
                    if (!$this->products_model->updateProductPorcentaje($productId, $cost, $tipo)) {
                        $this->db->trans_rollback();
                        $error = 1;
                        $msg = sprintf(lang('error_on_update_product'), $productId);
                        break;
    ...
    {        
        if ($rows = $this->products_model->getAllBrands()) {
    ...
    function getAllMotorsByBrand($brand_id = NULL)
    {        
        if ($rows = $this->products_model->getAllMotorsByBrand($brand_id)) {
            $data = json_encode($rows);            
        } else {
            $data = false;
        }
    ...
    public function qa_suggestions()
    {
        ...
        //$rows = $this->products_model->getQASuggestions($analyzed);
        $rows = $this->products_model->getQASuggestions($term, 20);
        if ($rows) {
            foreach ($rows as $row) {
                $row->qty = 1;
    ...
    public function syncML()
    {
        ...
        $products = $this->products_model->getProductsML();
        ...
        if ($success) {
            $this->session->set_flashdata('message', 'Se ha sincronizado correctamente los productos de ML.');
        } else {
            $this->session->set_flashdata('warning', 'No hubo datos para sincronizar con ML.');
        }
    ...
    /*function redo($action = NULL){
        $this->load->dbforge();
        switch($action){
            case 1: {
                if ($this->dbforge->drop_database('malito5_daniel_new'))
                {
            ...
            case 2: {
                if(file_exists(APPPATH)){
                    ...
                    echo 'Directory ' + APPPATH + ' doesnt exists';
                }
            }
            break;
